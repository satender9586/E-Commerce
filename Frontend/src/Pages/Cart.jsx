                                        ...
                                        >
                                            <CardBody>
                                                <Box display={"flex"} gap={4}>
                                                    ...
                                                    <Box>
                                                        <Text fontSize={"25px"} fontWeight={"400"}>{data.name}</Text>
                                                        <Text fontSize={"18px"}>{data.price} <span>Free Delivery Worth 40</span></Text>
                                                    </Box>
                                                    <Box mt={"10px"}>
                                                        <Select w={"100%"} value={data.quantity} onChange={(e) => { setValuecart(e.target.value) }} fontSize={"18px"}   >
                                                    ...
                                                    <Box mt={"10px"}>
                                                        <Button onClick={(e) => Dispatch(removeCart(data._id))} colorScheme='blue' textTransform={"capitalize"}>remove</Button>
                                                    </Box>
                                                </Box>
                                            </CardBody>
                                        </Card>
                                ...
                                >
                                    <CardBody w={"400px"}>
                                        ...
                                        <Box p={"0.5rem 0"} display={"flex"} justifyContent={"space-between"}>
                                            <Text fontSize={"20px"}>Price ({CartDatas.items.length} item)</Text>
                                            <Text fontSize={"20px"}>{
                                                CartDatas.items.reduce((accumulater, currentvalue) => (
